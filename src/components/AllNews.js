...
export class AllNews extends Component {
  ...
  async handlePrevious() {
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=${
      this.props.country
    }&category=${this.props.category}&apiKey=${process.env.REACT_APP_API_KEY}&page=${
      this.state.pg - 1
    }&pageSize=${this.props.pageSize}`;
    this.setState({ loading: true });
    let data = await fetch(url);
      ...
      {/* headline */}
        <div className="container my-3">
          <div className="text-center headline">
            ...
            <h1 className="py-2">{this.props.category.charAt(0).toUpperCase() +this.props.category.slice(1)}</h1>
          </div>
          {/* spinner */}
  ...
  async componentDidMount() {
    window.scrollTo(0, 0);
    const url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=${process.env.REACT_APP_API_KEY}&page=1&pageSize=${this.props.pageSize}`;
    this.setState({ loading: true });
    let data = await fetch(url);
    let parsedData = await data.json();
